...
The lag order $p$ is a *hyperparameter* that needs to be selected. $p$ determines the number of predictions in the (auto)regression model, and so selecting the lag order is analogous to variable selection in iid regression.
...
CV assesses out-of-sample performance and is highly customizeable, but can be more labor-intensive to implement.
