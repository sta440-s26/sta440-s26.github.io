...
for (t in 2:Tlen) {
  ...
  x_t <- matrix(c(1, rgdp[t-1]), nrow = p)
  ...
  res <- nig_update(rgdp[t], x_t, a, b, m, V)
  a <- res$a; b <- res$b; m <- res$m; V <- res$V
...
```{r}
#| echo: false
PI_iid_real <- compute_PI_coverage(rgdp, pred_params_iid_real, alpha = 0.10)

PI_ar1_real <- compute_PI_coverage(rgdp, pred_params_ar1_real, alpha = 0.10)
