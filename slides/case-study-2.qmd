...
[Check it out](https://www.philadelphiafed.org/surveys-and-data/real-time-data-research/real-time-data-set-for-macroeconomists)!


## What are these data like? {.small}
...
ROUTPUTQvQd_2 <- read_excel("data/ROUTPUTQvQd-2.xlsx", na = "#N/A")
...
## Rows are the history of revisions {.medium}
...
::::: columns
::: {.column width="50%"}
<iframe data-testid="embed-iframe" style="border-radius:12px" src="https://open.spotify.com/embed/track/3KfbEIOC7YIv90FIfNSZpo?utm_source=generator" width="100%" height="75" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
:::
...
![](images/real-time.gif){fig-align="center"}
...
vintage_to_frac <- function(v) {
  yr <- as.numeric(sub("v([0-9]{2})Q.*", "\\1", v))
  yr <- ifelse(yr >= 50, 1900 + yr, 2000 + yr)
  q <- as.numeric(sub(".*Q([1-4])", "\\1", v))
  yr + (q - 1)/4
}

vint_frac <- vintage_to_frac(colnames(output_growth)[2:ncol(output_growth)])

plot(vint_frac, 
     output_growth[100, 2:ncol(output_growth)], 
     type = "l",
     xlab = "Vintage",
     ylab = "YoY % Change",
     main = paste("Measuring GDP growth in", output_growth$DATE[100]),
     bty = "n",
     col = "red",
     lwd = 2)
#lines(vint_frac, output_growth[90, 2:ncol(output_growth)], type = "l")
#lines(vint_frac, output_growth[80, 2:ncol(output_growth)], type = "l")
```
...
![](images/predicting-revisions.png){fig-align="center" width="70%"}
...
- sampling uncertainty for unknown $\beta$.
...
- Aruoba (2008 JMCB): "[Data revisions are not well behaved](https://doi.org/10.1111/j.1538-4616.2008.00115.x);"
- Croushore (2011 JEL): "[Frontiers of real-time data analysis](https://www.aeaweb.org/articles?id=10.1257/jel.49.1.72);"
- Jacobs and van Norden (2016 JoM): "[Why are initial estimates of productivity growth so unreliable?](https://doi.org/10.1016/j.jmacro.2015.11.004)"

Check out [FRED](https://fred.stlouisfed.org) and the [FRBP's Real-Time Data Set](https://www.philadelphiafed.org/surveys-and-data/real-time-data-research/real-time-data-set-for-macroeconomists).
...
## Non-standard Student's $t$ {.small}
...
## Gird your loins! {.small .scrollable}
...
## Gird your loins! {.scrollable .small}
